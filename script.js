...
function HeaderComponent(){
    ...
    headerContainer.setAttribute("style","height:70px;");
    
    let logoDiv = document.createElement("div");
    logoDiv.setAttribute("style","width:25%;height:60px;");
    logoDiv.setAttribute("class","d-flex flex-column text-center justify-content-center")
    
    let logoLabel = document.createElement("label");
    logoLabel.setAttribute("style","cursor: pointer;")
    logoLabel.innerHTML = "<b class='text-white' style='font-size: 18px;'><div><span class='text-danger'>🛒 E-shop</span></div></b><small class='text-white'>Shop on a single click</small>"
    logoLabel.addEventListener("click", function(){
       if(isLoggedIn()){
    ...
    searchBox.setAttribute("style","width:40%;height:60px;");
    ...
    optionMenu.setAttribute("style","width:35%;height:60px;");
    optionMenu.setAttribute("class","d-flex text-white align-items-center justify-content-end pe-3")
    
    if(isLoggedIn()){
       let cartCount = getCartCount();
       let cartBadge = document.createElement("div");
       cartBadge.setAttribute("style","position: relative; margin-right: 20px;")
       let cartIcon = document.createElement("label");
       cartIcon.innerHTML = `<span style='font-size: 24px; cursor: pointer;'>🛍️</span>`;
       cartIcon.setAttribute("style","cursor: pointer; position: relative;")
       let badge = document.createElement("span");
       badge.setAttribute("class","badge bg-danger");
       badge.setAttribute("style","position: absolute; top: -8px; right: -8px;");
       ...
       optionMenu.appendChild(cartBadge);
    }
    
    let option1 = document.createElement("label");
    option1.innerText = isLoggedIn() ? "View cart" : "Sign in";
    option1.setAttribute("style","cursor: pointer; margin-right: 15px;")
    ...
    option2.setAttribute("style","margin-left:15px; cursor: pointer;")
...
function ViewCartComponent(mainContainer){
   ...
   container.setAttribute("class","container-fluid mt-4");
   container.setAttribute("id","card-container");
   container.setAttribute("style","background-color: #f8f9fa; padding: 20px; min-height: 100vh;");
   
   let topNav = document.createElement("div");
   topNav.setAttribute("class","mb-4 d-flex justify-content-between align-items-center");
   
   let heading = document.createElement("h2");
   heading.setAttribute("class","text-dark mb-0");
   ...
   let cartItems = usersCart[currentUser] || [];
   
   let cartBadge = document.createElement("span");
   cartBadge.setAttribute("class","badge bg-primary");
   cartBadge.setAttribute("style","font-size: 16px; padding: 8px 15px;");
   cartBadge.innerHTML = `${cartItems.length} Item${cartItems.length !== 1 ? 's' : ''}`;
   topNav.appendChild(cartBadge);
   
   container.appendChild(topNav);
   
   let containerRow = document.createElement("div");
   containerRow.setAttribute("class","row");
   containerRow.setAttribute("style","min-height:500px;");
   
   let containerItems = document.createElement("div");
   containerItems.setAttribute("class","col-md-8");
   containerItems.setAttribute("style","background: white; padding: 20px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);");
   
   if(cartItems.length === 0){
      let emptyMsg = document.createElement("div");
      emptyMsg.setAttribute("class","text-center");
      emptyMsg.setAttribute("style","padding: 60px 20px;");
      emptyMsg.innerHTML = `
         <div style="font-size: 60px; margin-bottom: 20px;">🛍️</div>
         <h4>Your cart is empty</h4>
         <p class="text-muted">Add products to get started!</p>
      `;
      containerItems.appendChild(emptyMsg);
   } else {
      let cartControls = document.createElement("div");
      cartControls.setAttribute("class","mb-3 d-flex justify-content-between align-items-center");
      cartControls.setAttribute("style","border-bottom: 2px solid #f0f0f0; padding-bottom: 15px;");
      
      let itemsInfo = document.createElement("span");
      itemsInfo.setAttribute("class","text-muted");
      itemsInfo.innerHTML = `Showing <strong>${cartItems.length}</strong> item${cartItems.length !== 1 ? 's' : ''}`;
      cartControls.appendChild(itemsInfo);
      
      let clearBtn = document.createElement("button");
      clearBtn.setAttribute("class","btn btn-sm btn-outline-danger");
      clearBtn.innerHTML = "🗑️ Clear Cart";
      clearBtn.addEventListener("click", function(){
         if(window.confirm("Are you sure you want to clear your cart?")){
            usersCart[currentUser] = [];
            localStorage.setItem("users-cart", JSON.stringify(usersCart));
            location.reload();
         }
      });
      cartControls.appendChild(clearBtn);
      containerItems.appendChild(cartControls);
      
      let table = document.createElement("table");
      table.setAttribute("class","table table-hover");
      let thead = document.createElement("thead");
      thead.setAttribute("class","table-dark");
      thead.innerHTML = "<tr><th>Product</th><th>Price</th><th>Qty</th><th>Total</th><th>Action</th></tr>";
      ...
      for(let item of cartItems){
         ...
         let productData = getData().find(p => p.id === item.id);
         let itemTotal = item.price * item.qty;
         subtotal += itemTotal;
         
         row.innerHTML = `
            <td><strong>${item.title.substring(0, 25)}</strong></td>
            <td>Rs. ${item.price.toFixed(2)}</td>
            <td>
               <input type="number" class="form-control" style="width: 70px;" value="${item.qty}" min="1" max="100"
                  onchange="updateCartQty(${item.id}, this.value, '${currentUser}')">
            </td>
            <td><strong>Rs. ${itemTotal.toFixed(2)}</strong></td>
            <td>
               <button class="btn btn-sm btn-danger" onclick="removeFromCart(${item.id}, '${currentUser}')">
                  ❌ Remove
               </button>
            </td>
         `;
   ...
   orderContainer.setAttribute("class","col-md-4 ps-md-3");
   orderContainer.setAttribute("style","background: white; padding: 25px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); height: fit-content;");
   
   let orderHeading = document.createElement("h5");
   orderHeading.setAttribute("class","mb-4 text-dark border-bottom pb-3");
   orderHeading.innerHTML = "💰 Order Summary";
   orderContainer.appendChild(orderHeading);
   
   let subtotalAmount = cartItems.reduce((sum, item) => sum + (item.price * item.qty), 0);
   let shippingCost = cartItems.length > 0 ? 100 : 0;
   let taxAmount = subtotalAmount * 0.1;
   let total = subtotalAmount + shippingCost + taxAmount;
   
   let subtotalLabel = document.createElement("div");
   subtotalLabel.setAttribute("class","d-flex justify-content-between mb-2");
   subtotalLabel.innerHTML = `<span>Subtotal:</span><strong>Rs. ${subtotalAmount.toFixed(2)}</strong>`;
   orderContainer.appendChild(subtotalLabel);
   
   let shippingLabel = document.createElement("div");
   shippingLabel.setAttribute("class","d-flex justify-content-between mb-2");
   shippingLabel.innerHTML = `<span>Shipping:</span><strong class='text-success'>Rs. ${shippingCost}</strong>`;
   orderContainer.appendChild(shippingLabel);
   
   let taxLabel = document.createElement("div");
   taxLabel.setAttribute("class","d-flex justify-content-between mb-3 pb-3 border-bottom");
   taxLabel.innerHTML = `<span>Tax (10%):</span><strong>Rs. ${taxAmount.toFixed(2)}</strong>`;
   orderContainer.appendChild(taxLabel);
   
   let totalLabel = document.createElement("div");
   totalLabel.setAttribute("class","d-flex justify-content-between mb-4");
   totalLabel.innerHTML = `<h5>Total:</h5><h5 class='text-success'>Rs. ${total.toFixed(2)}</h5>`;
   orderContainer.appendChild(totalLabel);
   
   // Coupon Section
   let couponSection = document.createElement("div");
   couponSection.setAttribute("class","mb-4 p-3 bg-light rounded");
   let couponLabel = document.createElement("label");
   couponLabel.setAttribute("class","mk-1 small text-muted");
   couponLabel.innerHTML = "Apply Coupon Code";
   couponSection.appendChild(couponLabel);
   let couponInput = document.createElement("input");
   couponInput.setAttribute("type","text");
   couponInput.setAttribute("class","form-control mb-2");
   couponInput.setAttribute("placeholder","Enter coupon code");
   couponSection.appendChild(couponInput);
   let couponBtn = document.createElement("button");
   couponBtn.setAttribute("class","btn btn-sm btn-outline-primary w-100");
   ...
   couponSection.appendChild(couponBtn);
   orderContainer.appendChild(couponSection);
   
   let checkoutBtn = document.createElement("button");
   checkoutBtn.setAttribute("class","btn btn-success btn-lg w-100 mb-2");
   checkoutBtn.innerHTML = "✅ Proceed to Checkout";
   checkoutBtn.addEventListener("click", function(){
      if(cartItems.length > 0){
         window.alert(`Order placed successfully! Total: Rs. ${total.toFixed(2)}\nThank you for your purchase!`);
         usersCart[currentUser] = [];
         localStorage.setItem("users-cart", JSON.stringify(usersCart));
         location.reload();
      } else {
   ...
   orderContainer.appendChild(checkoutBtn);
   
   let continueShopping = document.createElement("button");
   continueShopping.setAttribute("class","btn btn-outline-secondary btn-lg w-100 mb-2");
   continueShopping.innerHTML = "◀️ Continue Shopping";
   continueShopping.addEventListener("click", function(){
      container.innerHTML = "";
      cardContainer.remove();
      let newCardContainer = document.createElement("div");
      newCardContainer.setAttribute("id","card-container");
      document.querySelector("#main").appendChild(newCardContainer);
      CardComponent();
   });
   orderContainer.appendChild(continueShopping);
   
   let saveForLater = document.createElement("button");
   saveForLater.setAttribute("class","btn btn-outline-info btn-lg w-100");
   saveForLater.innerHTML = "❤️ Save for Later";
   saveForLater.addEventListener("click", function(){
      window.alert("Items saved for later!");
   });
   orderContainer.appendChild(saveForLater);
...
function SignUpComponent(mainContainer){
   ...
   signUpContainer.setAttribute("style","min-height:100vh; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);");
   ...
   signUpForm.setAttribute("style","width:100%; max-width:450px; background: white; border-radius: 10px;");
   signUpForm.setAttribute("class","p-4 shadow-lg");
   
   let heading = document.createElement("h2");
   heading.setAttribute("class","text-center mb-4 text-dark");
   ...
   contactInput.setAttribute("class","form-control mb-3");
   signUpForm.appendChild(contactInput);
   
   let passwordInput = document.createElement("input");
   passwordInput.type = "password";
   passwordInput.placeholder = "Enter password";
   passwordInput.setAttribute("class","form-control mb-4");
   signUpForm.appendChild(passwordInput);

   let submitButton = document.createElement("button");
   submitButton.innerText = "Sign Up";
   submitButton.setAttribute("class","btn btn-success btn-lg w-100 mb-3");
   ...
   let signInLink = document.createElement("p");
   signInLink.setAttribute("class","text-center text-muted mb-0");
   signInLink.innerHTML = "Already have an account? <a href='#' style='color: #667eea; text-decoration: none; font-weight: bold;'>Sign In</a>";
...
function SignInComponent(mainContainer){
   ...
   signInContainer.setAttribute("style","min-height:100vh; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);");
   ...
   signInForm.setAttribute("style","width:100%; max-width:450px; background: white; border-radius: 10px;");
   signInForm.setAttribute("class","p-4 shadow-lg");
   ...
   submitButton.setAttribute("class","btn btn-primary btn-lg w-100 mb-3");
   submitButton.addEventListener("click",function(){
      // Sign in logic
      let email = emailInput.value;
      let password = passwordInput.value;
   ...
   signUpLink.setAttribute("class","text-center text-muted mb-0");
   signUpLink.innerHTML = "Don't have an account? <a href='#' style='color: #667eea; text-decoration: none; font-weight: bold;'>Sign Up</a>";
   signUpLink.addEventListener("click", function(e){
      e.preventDefault();
      SignUpComponent(mainContainer);
   });
   signInForm.appendChild(signUpLink);
...
function CardComponent(){
   ...
   cardContainer.setAttribute("class","container-fluid mt-4");
   cardContainer.setAttribute("style","background-color: #f8f9fa; padding: 30px;");

   let heading = document.createElement("h3");
   heading.setAttribute("class","mb-4 text-dark");
   ...
   for(let product of productList){
     ...
     cardContent.setAttribute("class","card h-100 shadow-sm transition")
     cardContent.setAttribute("style","border: none; border-radius: 8px; overflow: hidden; cursor: pointer; transition: transform 0.3s;")
     cardContent.addEventListener("mouseenter", function(){
        cardContent.style.transform = "translateY(-5px)";
        cardContent.style.boxShadow = "0 8px 15px rgba(0,0,0,0.2)";
     });
     cardContent.addEventListener("mouseleave", function(){
        cardContent.style.transform = "translateY(0)";
        cardContent.style.boxShadow = "0 2px 5px rgba(0,0,0,0.1)";
     });
     
     let img = document.createElement("img");
     img.setAttribute("class","card-img-top");
     img.setAttribute("style","height:250px; width:100%; object-fit: cover;");
     ...
     rating.setAttribute("class","text-muted small mb-2");
     rating.innerHTML = `<span class='text-warning'>★ ${product.rating}</span> | Stock: <span class='${product.stock > 10 ? "text-success" : "text-danger"}'>${product.stock}</span>`;
     cardBody.appendChild(rating);
     
     let priceDiv = document.createElement("div");
     priceDiv.setAttribute("class","mb-3");
     let oldPrice = document.createElement("span");
     oldPrice.setAttribute("class","text-muted");
     oldPrice.setAttribute("style","text-decoration: line-through; font-size: 14px;");
     oldPrice.innerText = `Rs. ${product.price}`;
     let newPrice = document.createElement("span");
     newPrice.setAttribute("class","text-success ms-2");
     newPrice.setAttribute("style","font-size: 18px; font-weight: bold;");
     newPrice.innerText = `Rs. ${(product.price - (product.price * product.discountPercentage)/100).toFixed(2)}`;
     let discount = document.createElement("span");
     discount.setAttribute("class","badge bg-danger ms-2");
     discount.innerText = `-${product.discountPercentage.toFixed(0)}%`;
     ...
     buttonDiv.setAttribute("class","btn-group w-100");
     buttonDiv.setAttribute("role","group");
     
     let viewMoreButton = document.createElement("button");
     viewMoreButton.innerText = "View";
     viewMoreButton.setAttribute("class","btn btn-outline-primary btn-sm flex-grow-1");
     ...
     cartButton.setAttribute("class","btn btn-warning btn-sm flex-grow-1");
...
function ViewMoreComponent(product){
   ...
   viewMoreContainer.setAttribute("class","container-fluid");
   viewMoreContainer.setAttribute("style","background-color: #f8f9fa; padding: 30px;");
   ...
   leftDiv.setAttribute("style","background: white; padding: 20px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);");
   
   let img = document.createElement("img");
   img.src = product.thumbnail;
   img.setAttribute("style","width:100%; max-height:400px; object-fit: cover; border-radius: 8px; margin-bottom: 15px;");
   ...
   for(let imageUrl of product.images){
      ...
      smallImage.setAttribute("style","width:22%; height:70px; border-radius: 5px; cursor: pointer; border: 2px solid transparent;");
      smallImage.addEventListener("mouseenter", function(){
         smallImage.style.borderColor = "#FFC107";
      });
   ...
   rightDiv.setAttribute("class","col-md-6 ps-md-4");
   rightDiv.setAttribute("style","background: white; padding: 25px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);");
   
   let breadcrumb = document.createElement("nav");
   breadcrumb.setAttribute("class","small mb-3");
   breadcrumb.innerHTML = `<span class='text-muted'>${product.category} > ${product.brand}</span>`;
   rightDiv.appendChild(breadcrumb);
   ...
   brand.setAttribute("class","text-muted");
   brand.innerHTML = `<b>Brand:</b> <span class='text-info'>${product.brand}</span>`;
   rightDiv.appendChild(brand);
   
   let ratingDiv = document.createElement("div");
   ratingDiv.setAttribute("class","mb-3");
   ratingDiv.innerHTML = `<span class='text-warning'>★★★★☆ ${product.rating}/5.0</span> | <span class='text-success'>${product.availabilityStatus}</span>`;
   ...
   priceSection.setAttribute("class","mb-3 p-3 bg-light rounded");
   let originalPrice = document.createElement("p");
   originalPrice.setAttribute("class","mb-1");
   originalPrice.innerHTML = `<strikethrough class='text-muted'>Rs. ${product.price}</strikethrough>`;
   priceSection.appendChild(originalPrice);
   let discountedPrice = document.createElement("p");
   discountedPrice.setAttribute("class","mb-0");
   let finalPrice = (product.price - (product.price * product.discountPercentage)/100).toFixed(2);
   discountedPrice.innerHTML = `<h4 class='text-success'>Rs. <b>${finalPrice}</b></h4><small class='text-danger'>Save ${product.discountPercentage.toFixed(0)}% (Rs. ${(product.price * product.discountPercentage / 100).toFixed(2)})</small>`;
   priceSection.appendChild(discountedPrice);
   rightDiv.appendChild(priceSection);

   let infoSection = document.createElement("div");
   infoSection.setAttribute("class","mb-4");
   
   let label1 = document.createElement("p");
   label1.setAttribute("class","small mb-2");
   label1.innerHTML = "<b>🛡️ Warranty:</b> " + product.warrantyInformation;
   infoSection.appendChild(label1);

   let label2 = document.createElement("p");
   label2.setAttribute("class","small mb-2");
   label2.innerHTML = "<b>🚚 Shipping:</b> " + product.shippingInformation;
   infoSection.appendChild(label2);
   
   let label3 = document.createElement("p");
   label3.setAttribute("class","small mb-2");
   label3.innerHTML = "<b>↩️ Returns:</b> " + product.returnPolicy;
   infoSection.appendChild(label3);
   
   let label4 = document.createElement("p");
   label4.setAttribute("class","small");
   label4.innerHTML = "<b>📦 Stock:</b> <span class='" + (product.stock > 10 ? "text-success" : "text-danger") + "'>" + product.stock + " available</span>";
   infoSection.appendChild(label4);
   rightDiv.appendChild(infoSection);

   let buttonContainer = document.createElement("div");
   buttonContainer.setAttribute("class","d-grid gap-2 d-sm-flex");
   
   let addToCart = document.createElement("button");
   addToCart.setAttribute("class","btn btn-success btn-lg flex-grow-1");
   ...
   buyNow.setAttribute("class","btn btn-warning btn-lg flex-grow-1");
   buyNow.innerHTML = "⚡ Buy now";
   buyNow.addEventListener("click", function(){
      if(isLoggedIn()){
         let currentUser = sessionStorage.getItem("currentUser");
         let usersCart = JSON.parse(localStorage.getItem("users-cart"));
         usersCart[currentUser] = [{id: product.id, title: product.title, price: product.price, qty: 1}];
         localStorage.setItem("users-cart", JSON.stringify(usersCart));
...
function updateCartQty(productId, newQty, currentUser){
   let usersCart = JSON.parse(localStorage.getItem("users-cart"));
   let cartItems = usersCart[currentUser];
   ...
   if(item){
      ...
      usersCart[currentUser] = cartItems;
      localStorage.setItem("users-cart", JSON.stringify(usersCart));
      location.reload();
   }
}
function removeFromCart(productId, currentUser){
   let usersCart = JSON.parse(localStorage.getItem("users-cart"));
   let cartItems = usersCart[currentUser];
   usersCart[currentUser] = cartItems.filter(i => i.id !== productId);
   localStorage.setItem("users-cart", JSON.stringify(usersCart));
